...
import com.service.front.frontImpl.canteenServiceImpl;
...
@WebServlet(name = "orderCanteenServlet", value = "/orderCanteenServlet")
public class orderCanteenServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        //首先看用户是否已经选择景点，如果已经选择景点，则将景点周围200m距离的餐厅展示出来
        ...
        if(chooseAttraction==null){
            ...
            if(QueryCity==null||QueryCity==""){
                canteenList=canteenService.getBeginSomeInfo();
            }else{
                canteenList=canteenService.getSomeByQueryCity(QueryCity);
            }
            ...
            List<Integer> canteens=canteenService.getCanteensWithAttraction((Integer.parseInt(chooseAttraction)) ,Integer.parseInt(ahDistance));
            //根据查到所选择景点给定距离的附近酒店hid，来查询这些酒店的具体信息，存放到hotelList中，然后展示的前端页面中
            if(!canteens.isEmpty()) //如果查到景点附近的酒店了再去查这些酒店的具体信息，否则不去查询酒店
                canteenList=canteenService.getCanteensByHids(canteens);
            else
                canteenList=null;
        }
        //将查询到的酒店数据存放到request域对象中
        request.setAttribute("canteenList",canteenList);
        request.getRequestDispatcher("userjsp/userCanteen.jsp").forward(request,response);
    }
